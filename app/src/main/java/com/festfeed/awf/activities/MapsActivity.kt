...
import kotlinx.android.synthetic.main.activity_maps.backBtn
...
class MapsActivity : BaseActivity(), OnMapReadyCallback {
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        var place: LatLng
        mMap.run {
            for (location in selectedData.locations) {
                place = LatLng(location.latitude, location.longitude)
                val marker = mMap.addMarker(MarkerOptions().position(place).title(location.name))
            ...
            val padding = 50 // offset from edges of the eventmap in pixels
            ...
            // Add an overlay to the eventmap, retaining a handle to the GroundOverlay object.
